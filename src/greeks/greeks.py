...
def compute_greeks(
    ...
    try:
        ...
        def price_cache() -> (
            ...
            def _price(
                S_: float, K_: float, T_: float, r_: float, sigma_: float, q_: float
            ) -> float:
                key = (S_, K_, T_, r_, sigma_, q_)
                if key not in cache:
                    cache[key] = model.price(
                        S_,
                        K_,
                        T_,
                        r_,
        ...
        greeks["delta"] = _delta(
            model, S, K, T, r, sigma, option_type, exercise_style, q, h, P
        )
        greeks["gamma"] = _gamma(
            model, S, K, T, r, sigma, option_type, exercise_style, q, h, P
        )
        ...
        greeks["theta"] = _theta(
            model, S, K, T, r, sigma, option_type, exercise_style, q, P
        )
        greeks["rho"] = _rho(
            model, S, K, T, r, sigma, option_type, exercise_style, q, h, P
        )
...
def _delta(model, S, K, T, r, sigma, option_type, exercise_style, q, h, P) -> float:
    ...
    if exercise_style == ExerciseStyle.AMERICAN:
        ...
        option_values = model._backward_induction(
            asset_prices, K, r, dt, p, option_type.value, exercise_style.value
        )
    ...
    if exercise_style == ExerciseStyle.AMERICAN:
        if model.num_steps < 3:
            raise InputValidationError(
                "Gamma for American options requires num_steps >= 3"
            )
        u, d, p, dt = model._compute_params(S, K, T, r, sigma, q)
        asset_prices = model._compute_asset_prices(S, u, d, model.num_steps)
        option_values = model._backward_induction(
...
def _charm(model, S, K, T, r, sigma, option_type, exercise_style, q, h, P) -> float:
    ...
    delta_future = _delta(
        model, S, K, max(T - dt, 1e-8), r, sigma, option_type, exercise_style, q, h, P
    )
