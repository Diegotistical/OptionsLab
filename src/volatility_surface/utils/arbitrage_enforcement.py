...
def check_monotonicity(y: np.ndarray, axis: int = 1) -> np.ndarray:
    ...
    pad_shape[axis] = 1
    violations_padded = np.concatenate(
        [violations, np.zeros(pad_shape, dtype=bool)], axis=axis
    )
...
def _project_to_convex(
    seq: np.ndarray, max_iter: int = 50, tol: float = 1e-8
) -> np.ndarray:
    """
    ...
    for iteration in range(max_iter):
        ...
        logger.warning(
            "Max iterations reached in convexity projection without full convergence"
        )
    return y
