...
def _engineer_tensor(tensor: torch.Tensor) -> torch.Tensor:
    ...
    return torch.stack(
        [moneyness, log_moneyness, T, ttm_squared, r, vol, vol_skew], dim=1
    )
...
def validate_inputs(data: Union[pd.DataFrame, torch.Tensor]) -> None:
    """
    ...
    if isinstance(data, pd.DataFrame):
        ...
        if data.ndim != 2 or data.shape[1] < 5:
            raise ValueError(
                "Input tensor must have shape [batch_size, 5] (S, K, T, r, vol)"
            )
