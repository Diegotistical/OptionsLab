...
class VolatilitySurfaceGenerator:
    ...
    def __init__(
        ...
        *,
        ...
        assert (
            len(strikes) == len(maturities) == len(ivs)
    ...
    def _validate_inputs(self) -> None:
        if self.interp_method not in {"linear", "cubic", "nearest"}:
            raise ValueError(
                f"Interpolation method '{self.interp_method}' not supported"
            )
    ...
    def generate_surface(self) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        """
        with self._lock:
            self.grid_strikes = np.linspace(
                self.strikes.min(), self.strikes.max(), self.strike_points
            )
            self.grid_maturities = np.linspace(
                self.maturities.min(), self.maturities.max(), self.maturity_points
            )
            grid_strike_mesh, grid_maturity_mesh = np.meshgrid(
            ...
            if val is None or np.isnan(val):
                if self.allow_extrapolation:
                    val = _numba_nearest(
                        ...
                        self.maturities,
                        self.ivs,
                        np.array([[strike, maturity]]),
                    )
                else:
                    raise ValueError(
                        f"Point (strike={strike}, maturity={maturity}) outside interpolation domain"
                    )
            return float(val[0])

    def get_surface_batch(
        ...
        with self._lock:
            ...
            if new_points:
                ...
                if self.use_gpu:
                    ...
                    new_values = _gpu_nearest(
                        strikes_gpu, maturities_gpu, ivs_gpu, coords_gpu
                    )
                    new_values = cp.asnumpy(new_values)
                elif self.use_numba and self.interp_method == "nearest":
                    new_values = _numba_nearest(
                        self.strikes, self.maturities, self.ivs, new_coords
                    )
                else:
                    new_values = griddata(
                        np.vstack((self.strikes, self.maturities)).T,
    ...
    def update_data(
        self, strikes: np.ndarray, maturities: np.ndarray, ivs: np.ndarray
    ) -> None:
...
@njit
def _numba_nearest(
    strikes: np.ndarray, maturities: np.ndarray, ivs: np.ndarray, queries: np.ndarray
) -> np.ndarray:
...
if __name__ == "__main__":
    ...
    queries = np.column_stack(
        (np.random.uniform(50, 150, 10000), np.random.uniform(0.01, 2.0, 10000))
    )
