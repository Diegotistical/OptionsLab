...
class ONNXExporter:
        ...
        for name, module in model.named_modules():
            if hasattr(module, "training") and module.training:
                export_warnings.append(
                    f"Submodule '{name}' is in training mode"
                )
        
        # Set up names
        input_names = input_names or ["input"]
        output_names = output_names or ["output"]
        feature_names = feature_names or [f"feature_{i}" for i in range(dummy_input.shape[-1])]
    ...
    def export_lightgbm(
        ...
        try:
            ...
            raise ONNXExportError(
                "onnxmltools required: pip install onnxmltools"
            )
    ...
    def export_sklearn(
        ...
        try:
            ...
            raise ONNXExportError(
                "skl2onnx required: pip install skl2onnx"
            )
