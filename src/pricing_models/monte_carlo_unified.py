...
class MonteCarloPricerUni:
    ...
    def _simulate_terminal_prices(
        self, S_arr, T_arr, r_arr, sigma_arr, q_arr
    ) -> np.ndarray:
        """
        ...
        if self.use_gpu:
            Z = cp.asarray(
                self.rng.normal(size=(n, self.num_simulations, self.num_steps))
            )
            Z_ant = -Z
    ...
    def delta_gamma(
        self, S, K, T, r, sigma, option_type: Literal["call", "put"], q=0.0, h=1e-4
    ):
        """
        ...
        prices = np.array(
            [self.price(S_i, K, T, r, sigma, option_type, q) for S_i in S_arr]
        )
...
if __name__ == "__main__":
    ...
    mc_pricer_cpu = MonteCarloPricerUni(
        num_simulations=50_000, use_numba=True, use_gpu=False
    )
    mc_pricer_gpu = MonteCarloPricerUni(
        num_simulations=50_000, use_numba=True, use_gpu=True
    )
    ...
    print(
        f"CPU MC Price: {price_cpu:.4f}, Delta: {delta_cpu:.4f}, Gamma: {gamma_cpu:.6f}, Time: {cpu_time:.2f}s"
    )
    ...
    if GPU_AVAILABLE:
        ...
        print(
            f"GPU MC Price: {price_gpu:.4f}, Delta: {delta_gpu:.4f}, Gamma: {gamma_gpu:.6f}, Time: {gpu_time:.2f}s"
        )
    else:
    ...
    terminal_prices = mc_pricer_cpu._simulate_terminal_prices(
        np.array([S]), np.array([T]), np.array([r]), np.array([sigma]), np.array([q])
    )[0]
    ...
    plt.title(
        f"Monte Carlo Terminal Price Distribution\nS={S}, K={K}, T={T}, sigma={sigma}"
    )
...
if __name__ == "__main__":
    ...
    for i in range(S_mesh.shape[0]):
        for j in range(S_mesh.shape[1]):
            ...
            price_ml_grid[i, j] = ml_model.predict(
                pd.DataFrame(
                    {
                        "S": [S_mesh[i, j]],
                        "K": [K],
    ...
    ax1.plot_surface(
        S_mesh, T_mesh, price_mc_grid, cmap="viridis", edgecolor="k", alpha=0.8
    )
    ...
    ax2.plot_surface(
        S_mesh, T_mesh, price_ml_grid, cmap="plasma", edgecolor="k", alpha=0.8
    )
