...
try:
    from components import (
        ...
        get_chart_layout, format_price, format_time_ms
    )
except ImportError:
...
try:
    from src.pricing_models import (
        black_scholes, MonteCarloPricer, MonteCarloPricerUni,
        MonteCarloMLSurrogate, BinomialTree,
        NUMBA_AVAILABLE, GPU_AVAILABLE, LIGHTGBM_AVAILABLE
    )
...
def run_pricing_benchmarks(
    ...
    option_type: str, num_sims: int, use_numba: bool
) -> list:
    ...
    try:
        ...
        results.append({"model": "Black-Scholes", "price": 0, "time_ms": 0, "std_ms": 0, "status": f"❌ {e}"})
    ...
    try:
        ...
        results.append({"model": "MC Basic", "price": 0, "time_ms": 0, "std_ms": 0, "status": f"❌ {e}"})
    ...
    try:
        ...
        results.append({"model": "MC Unified", "price": 0, "time_ms": 0, "std_ms": 0, "status": f"❌ {e}"})
    ...
    try:
        ...
        results.append({"model": "Binomial Tree", "price": 0, "time_ms": 0, "std_ms": 0, "status": f"❌ {e}"})
    ...
    try:
        ...
        results.append({"model": "ML Surrogate", "price": 0, "time_ms": 0, "std_ms": 0, "status": f"❌ {e}"})
...
with col1:
    st.markdown(f"**Numba JIT:** {'✅ Available' if NUMBA_AVAILABLE else '❌ Not installed'}")
with col2:
    st.markdown(f"**GPU (CuPy):** {'✅ Available' if GPU_AVAILABLE else '❌ Not installed'}")
with col3:
    st.markdown(f"**LightGBM:** {'✅ Available' if LIGHTGBM_AVAILABLE else '❌ Using sklearn'}")
...
with col3:
    ...
    num_sims = st.select_slider("MC Simulations", options=[10000, 25000, 50000, 100000], value=50000, key="bm_sims")
    use_numba = st.checkbox("Enable Numba", value=NUMBA_AVAILABLE, disabled=not NUMBA_AVAILABLE, key="bm_numba")
...
if run_basic:
    ...
    with st.spinner("Running benchmarks..."):
        ...
        results = run_pricing_benchmarks(S, K, T, r, sigma, option_type, num_sims, use_numba)
    ...
    fastest = min([r["time_ms"] for r in results if r["status"] == "✅" and r["time_ms"] > 0])
    ...
    with tab1:
        ...
        with col1:
            # Speed comparison
            valid_results = [r for r in results if r["status"] == "✅" and r["time_ms"] > 0]
            
            fig = go.Figure()
            fig.add_trace(go.Bar(
                x=[r["model"] for r in valid_results],
                y=[r["time_ms"] for r in valid_results],
                error_y=dict(type='data', array=[r["std_ms"] for r in valid_results]),
                marker_color=['#3b82f6', '#60a5fa', '#8b5cf6', '#a78bfa', '#10b981'],
            ))
        ...
        with col2:
            # Price comparison
            bs_price = next((r["price"] for r in results if "Black-Scholes" in r["model"]), None)
    ...
    with tab2:
        ...
        st.dataframe(
            df[["model", "price", "time_ms", "std_ms", "status"]].rename(columns={
                "model": "Model",
                "price": "Price ($)",
                "time_ms": "Avg Time (ms)",
                "std_ms": "Std Dev (ms)",
...
if run_scaling:
    ...
    with st.spinner("Running scaling tests..."):
        ...
        for n in sim_counts:
            ...
            try:
                ...
                _, t, _ = benchmark_function(pricer.price, S, K, T, r, sigma, option_type, n_trials=2)
    ...
    if mc_times_numba and all(t is not None for t in mc_times_numba):
        speedups = [np_t / nb_t for np_t, nb_t in zip(mc_times_numpy, mc_times_numba) if nb_t]
...
if not run_basic and not run_scaling:
    st.info("👆 Configure parameters and run benchmarks to compare pricing engine performance.")
