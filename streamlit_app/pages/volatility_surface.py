...
# HEADER
# =============================================================================
page_header("Volatility Surface", "ML-based implied volatility modeling and visualization")
...
with col1:
    ...
    atm_vol = st.slider("ATM Volatility (%)", min_value=5, max_value=60, value=20, key="vs_atm") / 100
    skew = st.slider("Skew", min_value=-30, max_value=10, value=-10, key="vs_skew") / 100

with col2:
    st.markdown("**Surface Shape**")
    smile = st.slider("Smile Curvature", min_value=0, max_value=20, value=5, key="vs_smile") / 100
    term_slope = st.slider("Term Slope", min_value=-10, max_value=5, value=-2, key="vs_term") / 100

with col3:
    st.markdown("**Grid Resolution**")
    n_strikes = st.select_slider("Strike Points", options=[10, 15, 20, 30, 40], value=20, key="vs_strikes")
    n_maturities = st.select_slider("Maturity Points", options=[8, 10, 15, 20, 25], value=15, key="vs_mat")
...
with col4:
    ...
    seed = st.number_input("Random Seed", min_value=1, max_value=99999, value=42, key="vs_seed")
...
if generate_btn:
    ...
    st.success(f"✅ Generated {n_strikes * n_maturities} data points in {format_time_ms(t_gen)}")
...
if train_btn:
    if st.session_state.iv_data is None:
        ...
        st.success(f"✅ {model_name} trained in {format_time_ms(t_train)} | R²: {r2:.4f} | RMSE: {rmse:.4f}")
...
if st.session_state.iv_data is not None:
    ...
    with tab3:
        ...
        with col1:
            ...
            for idx in [0, len(maturities)//4, len(maturities)//2, 3*len(maturities)//4, -1]:
                fig.add_trace(go.Scatter(
                    x=moneyness,
                    y=iv_grid[idx] * 100,
                    mode='lines',
                    line=dict(width=2.5, color=colors[min(idx, len(colors)-1) if idx >= 0 else -1]),
        ...
        with col2:
            ...
            for idx in [0, len(moneyness)//4, len(moneyness)//2, 3*len(moneyness)//4, -1]:
                fig.add_trace(go.Scatter(
                    x=maturities,
                    y=iv_grid[:, idx] * 100,
                    mode='lines',
    ...
    st.info("👆 Click **Generate Data** to create a synthetic IV surface, then **Train & Fit** to model it with ML.")
