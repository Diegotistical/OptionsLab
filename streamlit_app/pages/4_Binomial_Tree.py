...
page_header(
    ...
    "Cox-Ross-Rubinstein model for European and American options",
)
...
with col3:
    ...
    r_pct = st.number_input(
        "Rate (%)", min_value=0.0, max_value=20.0, value=5.0, step=0.5, key="bt_rate"
    )
    sigma_pct = st.number_input(
        "Vol (%)", min_value=1.0, max_value=100.0, value=20.0, step=1.0, key="bt_vol"
    )
...
with col4:
    ...
    n_steps = st.select_slider(
        "Steps", options=[10, 25, 50, 100, 200, 500, 1000], value=100, key="bt_steps"
    )
...
if run:
    ...
    tab1, tab2, tab3 = st.tabs(
        ["📈 Convergence", "🌳 Tree Structure", "📊 Sensitivity"]
    )
    ...
    with tab2:
        ...
        fig.update_layout(
            **get_chart_layout(f"Binomial Tree ({viz_steps} steps shown)", 450)
        )
        ...
        if n_steps > viz_steps:
            st.caption(
                f"Note: Only first {viz_steps} steps shown. Full tree has {n_steps} steps."
            )
    ...
    st.info(
        "👆 Configure parameters and click **Price Option** to run binomial tree pricing."
    )
