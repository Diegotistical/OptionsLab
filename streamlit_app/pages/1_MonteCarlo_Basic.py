...
try:
    from src.pricing_models import MonteCarloPricer, black_scholes, NUMBA_AVAILABLE, GPU_AVAILABLE
except ImportError:
...
# HEADER
# =============================================================================
page_header("Monte Carlo Option Pricing", "Simulate option prices with vectorized Monte Carlo simulation")
...
with col1:
    ...
    S = st.number_input("Spot Price ($)", min_value=1.0, max_value=1000.0, value=100.0, step=1.0)
    K = st.number_input("Strike Price ($)", min_value=1.0, max_value=1000.0, value=100.0, step=1.0)

with col2:
    st.markdown("**Time & Type**")
    T = st.number_input("Time to Maturity (years)", min_value=0.01, max_value=5.0, value=1.0, step=0.05)
    option_type = st.selectbox("Option Type", ["call", "put"])

with col3:
    st.markdown("**Market Conditions**")
    r_pct = st.number_input("Risk-Free Rate (%)", min_value=0.0, max_value=20.0, value=5.0, step=0.5)
    sigma_pct = st.number_input("Volatility (%)", min_value=1.0, max_value=200.0, value=20.0, step=1.0)
    r = r_pct / 100.0
...
with col4:
    ...
    num_sims = st.select_slider(
        "Simulations",
        options=[10000, 20000, 30000, 40000, 50000, 75000, 100000, 150000, 200000, 250000],
        value=50000
    )
    num_steps = st.select_slider("Time Steps", options=[25, 50, 100, 200, 500], value=100)
    
    accel_col1, accel_col2 = st.columns(2)
    with accel_col1:
        use_numba = st.checkbox("Numba JIT", value=NUMBA_AVAILABLE, disabled=not NUMBA_AVAILABLE,
                                 help="Enable Numba JIT for faster CPU execution")
    with accel_col2:
        use_gpu = st.checkbox("GPU", value=False, disabled=not GPU_AVAILABLE,
...
if run:
    ...
    # Charts
    tab1, tab2, tab3 = st.tabs(["📈 Price Paths", "🎯 Payoff Profile", "📊 Greeks Sensitivity"])
    ...
    with tab1:
        ...
        for t_idx in range(num_steps):
            ...
            paths[:, t_idx + 1] = paths[:, t_idx] * np.exp((r - 0.5 * sigma**2) * dt + sigma * np.sqrt(dt) * z)
        ...
        fig.add_hline(y=K, line_dash="dash", line_color="#ef4444", annotation_text=f"Strike: ${K}")
    ...
    with tab2:
        ...
        # Use BS for speed
        if black_scholes:
            option_values = np.array([black_scholes(s, K, T, r, sigma, option_type) for s in spot_range])
        else:
        ...
        fig.add_vline(x=S, line_dash="dot", line_color="#10b981", annotation_text="Current")
