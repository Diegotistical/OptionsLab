...
def get_monte_carlo_ml():
    ...
    class MonteCarloML:
        ...
        def generate_training_data(self, df):
            """Generate training data using MC pricing with common random numbers for Greeks"""
            X = df.copy()
            ...
            for _, row in X.iterrows():
                try:
                    ...
                    # Calculate approximate Greeks using common random numbers
                    h = max(1e-4 * S, 1e-5)  # Adaptive step size
                    num_sim_temp = max(self.num_simulations, 100000)  # Increase simulations for Greeks
                    
                    # Generate paths ONCE and reuse for all three points
                    np.random.seed(self.seed + 1)
                    Z_greeks = np.random.standard_normal((num_sim_temp, self.num_steps))
                    ...
                    for t in range(1, self.num_steps):
                        drift = (r - q - 0.5 * sigma**2) * dt
                        diffusion = sigma * np.sqrt(dt) * Z_greeks[:, t]
                    ...
                    # VALIDATION: Ensure delta is in proper range for calls
                    if delta < 0:
        ...
        def predict(self, X):
            ...
            for _, row in X.iterrows():
                try:
                    ...
                    # Calculate Greeks using common random numbers (proper implementation)
                    h = max(1e-4 * S, 1e-5)  # Adaptive step size
                    num_sim_temp = max(self.num_simulations, 100000)  # Increase simulations for Greeks
...
# Robust fallback Greeks calculation with common random numbers
def greeks_mc_delta_gamma(S, K, T, r, sigma, option_type, q=0.0, num_sim=50000, num_steps=100, seed=42, h=None, use_numba=False):
    """Robust fallback implementation that never returns None with proper Greeks calculation"""
    try:
        ...
        num_sim_temp = max(num_sim, 100000)
        
        # Generate random numbers ONCE and reuse for all three points
        np.random.seed(seed)
        dt = T / num_steps
        Z = np.random.standard_normal((num_sim_temp, num_steps))
        
        # Generate paths for all three points
        S_paths_up = np.zeros((num_sim_temp, num_steps))
        S_paths_down = np.zeros((num_sim_temp, num_steps))
        S_paths_mid = np.zeros((num_sim_temp, num_steps))
...
if train:
    try:
        ...
        try:
            ...
            # Calculate Greeks with proper common random numbers implementation
            delta_mc, gamma_mc = greeks_mc_delta_gamma(
                S, K, T, r, sigma, option_type, q,
                num_sim=num_sim, num_steps=num_steps, seed=seed
            )
        except Exception as e:
        ...
        delta_error = abs(delta_ml - delta_mc) if option_type == "call" else 0.0
        gamma_error = abs(gamma_ml - gamma_mc) if option_type == "call" else 0.0
        ...
        with tab1:
            ...
            if option_type == "call":
                ...
                metrics_data["Monte Carlo"].extend([f"{delta_mc:.4f}", f"{gamma_mc:.6f}"])
