...
try:
    from src.pricing_models import GPU_AVAILABLE, NUMBA_AVAILABLE, MonteCarloPricerUni
except ImportError:
...
page_header(
    "Unified Monte Carlo",
    "High-performance pricing with Numba JIT and GPU acceleration",
)
...
with col3:
    ...
    r_pct = st.number_input(
        "Rate (%)", min_value=0.0, max_value=20.0, value=5.0, step=0.5, key="uni_rate"
    )
    sigma_pct = st.number_input(
        "Vol (%)", min_value=1.0, max_value=100.0, value=20.0, step=1.0, key="uni_vol"
    )
...
if run_single:
    ...
    with st.spinner("Running simulation..."):
        pricer = MonteCarloPricerUni(
            num_simulations=num_sims, num_steps=100, use_numba=use_numba, use_gpu=False
        )
...
if run_surface:
    ...
    for i, t_val in enumerate(time_range):
        progress.progress(
            int((i + 1) / n_time * 80), text=f"Computing T={t_val:.2f}..."
        )
        ...
        prices = pricer.price_batch(
            S_vals, K_vals, T_vals, r_vals, sigma_vals, option_type
        )
        deltas, _ = pricer.delta_gamma_batch(
            S_vals, K_vals, T_vals, r_vals, sigma_vals, option_type
        )
...
if not run_single and not run_surface:
    st.info(
        "👆 Configure parameters and click **Single Price** for one calculation or **Price Surface** for full grid."
    )
