    ...
    class FallbackPricer:
        ...
        def delta_gamma(self, S, K, T, r, sigma, option_type, q=0.0, h=None):
            ...
            if option_type == "call":
                ...
                if delta < 0:
                    logger.warning(f"Negative delta ({delta}) for call option. Clamping to 0.")
                    delta = 0.0
                elif delta > 1:
                    logger.warning(f"Delta ({delta}) > 1 for call option. Clamping to 1.")
                    delta = 1.0
            else:  # put
                # Put delta must be between -1 and 0
                if delta > 0:
                    logger.warning(f"Positive delta ({delta}) for put option. Clamping to 0.")
                    delta = 0.0
                elif delta < -1:
                    logger.warning(f"Delta ({delta}) < -1 for put option. Clamping to -1.")
                    delta = -1.0
...
def create_greeks_heatmap(Sg: np.ndarray, Tg: np.ndarray, 
                          deltas: np.ndarray, gammas: np.ndarray,
                          K: float, option_type: str) -> tuple:
...
if run:
    try:
        ...
        with tab3:
            ...
            # Gamma sensitivity
            st.markdown('<h3 class="subsection-header">Gamma Sensitivity</h3>', unsafe_allow_html=True)
        ...
        with tab4:
            ...
            for size in sim_sizes:
                try:
                    ...
                    delta_val, gamma_val = mc.delta_gamma(S, K, T, r, sigma, option_type, q)
                    elapsed = (time.perf_counter() - start) * 1000.0
            ...
            # Price convergence
            with col1:
                st.markdown('<h3 class="subsection-header">Price Convergence</h3>', unsafe_allow_html=True)
                fig_price_conv = go.Figure()
                fig_price_conv.add_trace(go.Scatter(
                    x=sim_sizes,
                    y=prices,
                    mode='lines+markers',
                    name='Price',
            ...
            # Delta convergence
            with col2:
                st.markdown('<h3 class="subsection-header">Delta Convergence</h3>', unsafe_allow_html=True)
                fig_delta_conv = go.Figure()
                fig_delta_conv.add_trace(go.Scatter(
                    x=sim_sizes,
                    y=deltas,
                    mode='lines+markers',
                    name='Delta',
            ...
            # Gamma convergence
            with col3:
                st.markdown('<h3 class="subsection-header">Gamma Convergence</h3>', unsafe_allow_html=True)
                fig_gamma_conv = go.Figure()
                fig_gamma_conv.add_trace(go.Scatter(
                    x=sim_sizes,
                    y=gammas,
                    mode='lines+markers',
                    name='Gamma',
            ...
            col1.markdown('<div style="background-color: #1E293B; border-radius: 8px; padding: 1rem;">', unsafe_allow_html=True)
            col1.markdown('<div style="color: #94A3B8; font-size: 0.85rem; font-weight: 500;">Execution Time (50k sims)</div>', unsafe_allow_html=True)
            col1.markdown(f'<div style="color: white; font-size: 1.5rem; font-weight: 600; margin-top: 0.3rem;">{t_ms:.2f} ms</div>', unsafe_allow_html=True)
            col1.markdown('</div>', unsafe_allow_html=True)
            
            col2.markdown('<div style="background-color: #1E293B; border-radius: 8px; padding: 1rem;">', unsafe_allow_html=True)
            col2.markdown('<div style="color: #94A3B8; font-size: 0.85rem; font-weight: 500;">Speedup Factor</div>', unsafe_allow_html=True)
            col2.markdown(f'<div style="color: white; font-size: 1.5rem; font-weight: 600; margin-top: 0.3rem;">{speedup:.1f}x</div>', unsafe_allow_html=True)
