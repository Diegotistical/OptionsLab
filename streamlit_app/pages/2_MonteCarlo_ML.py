...
# =============================================================================
page_header(
    "ML Surrogate Model", "Train LightGBM models for instant option pricing predictions"
)
...
with col1:
    st.markdown(
        f"**LightGBM:** {'✅ Available' if LIGHTGBM_AVAILABLE else '⚠️ sklearn fallback'}"
    )
with col2:
    st.markdown(
        f"**Optuna:** {'✅ Available' if OPTUNA_AVAILABLE else '❌ Not installed'}"
    )
with col3:
    st.markdown(
        f"**ONNX Runtime:** {'✅ Available' if ONNX_AVAILABLE else '❌ Not installed'}"
    )
...
with col3:
    ...
    n_estimators = st.select_slider(
        ...
        value=min(
            [100, 200, 300, 400, 500, 750, 1000], key=lambda x: abs(x - default_trees)
        ),
        key="ml_trees",
    )
    max_depth = st.slider(
        "Tree Depth", min_value=3, max_value=12, value=default_depth, key="ml_depth"
    )
...
with col4:
    ...
    st.write("")
    train_btn = st.button(
        "🎯 Train & Predict", type="primary", use_container_width=True
    )
...
if train_btn:
    ...
    surrogate.fit(n_samples=n_samples, option_type=option_type, verbose=False)
    ...
    if pct_err > 5:
        st.warning(
            f"⚠️ ML prediction error is {pct_err:.1f}%. Try increasing training samples or tree count."
        )
    ...
    tab1, tab2, tab3 = st.tabs(
        ["📊 Price Comparison", "🎨 Price Surface", "📈 Error Analysis"]
    )
    ...
    with tab1:
        ...
        fig.add_vline(
            x=S, line_dash="dot", line_color="#f97316", annotation_text="Current"
        )
    ...
    with tab3:
        ...
        with col1:
            ...
            fig = go.Figure()
            fig.add_trace(
                go.Histogram(x=errors, nbinsx=25, marker_color="#a78bfa", opacity=0.8)
            )
    ...
    st.info(
        "👆 Configure parameters and click **Train & Predict** to train the ML surrogate model."
    )
...
if OPTUNA_AVAILABLE or ONNX_AVAILABLE:
    adv_tab1, adv_tab2 = st.tabs(
        ["⚡ Hyperparameter Optimization", "📦 Model Export (ONNX)"]
    )
            ...
            **Optuna Hyperparameter Tuning**
            ...
            with opt_col1:
                ...
                opt_study_name = st.text_input(
                    "Study Name", value="mc_ml_study", key="opt_study_name"
                )
            ...
            with opt_col2:
                ...
                opt_seed = st.number_input(
                    "Random Seed", min_value=0, max_value=9999, value=42, key="opt_seed"
                )

            optimize_btn = st.button(
                "🚀 Run Optimization Study", type="primary", key="run_optuna"
            )
            ...
            if optimize_btn:
                with st.spinner(f"Running {n_trials} Optuna trials..."):
                    try:
                        ...
                        # Display results
                        st.success(
                            f"✅ Optimization complete! Best score: {result.best_value:.6f}"
                        )
                        ...
                        st.info(
                            f"📁 Results saved to `models/optimization_results/{opt_study_name}_result.json`"
                        )
            ...
            st.warning(
                "⚠️ Optuna not installed. Run `pip install optuna` to enable hyperparameter optimization."
            )
            ...
            **ONNX Model Export**
            ...
            if "optuna_result" in st.session_state:
                ...
                export_btn = st.button(
                    "📦 Export to ONNX", type="primary", key="export_onnx"
                )
                ...
                if export_btn:
                    with st.spinner("Exporting model to ONNX..."):
                        try:
                            ...
                            model.fit(
                                n_samples=10000, option_type="call", verbose=False
                            )
                            ...
                            if export_result.success:
                                ...
                                st.markdown(
                                    f"**Model size:** {export_result.model_size_bytes / 1024:.1f} KB"
                                )

                                # Validate
                                st.markdown("**Validation:**")
                                test_X = np.random.rand(100, len(feature_names)).astype(
                                    np.float32
                                )
                                ...
                                if validation.passed:
                                    st.success(
                                        f"✅ Validation passed (correlation: {validation.pearson_correlation:.6f})"
                                    )
                                else:
                                    st.warning(
                                        f"⚠️ Validation warning: {validation.diagnostics}"
                                    )
                            else:
                ...
                st.info(
                    "👆 Run an optimization study first to get optimized hyperparameters for export."
                )
            ...
            onnx_files = (
                list((ROOT / "models" / "saved_models").glob("*.onnx"))
                if (ROOT / "models" / "saved_models").exists()
                else []
            )
            ...
            st.warning(
                f"⚠️ Missing dependencies: {', '.join(missing)}. Run `pip install {' '.join(missing)}`"
            )
