...
def get_monte_carlo_ml():
    ...
    try:
        ...
        logger.info(
            "Successfully imported MonteCarloML from pricing_models.monte_carlo_ml"
        )
        return MonteCarloML
    ...
    try:
        ...
        logger.info(
            "Successfully imported MonteCarloML from src.pricing_models.monte_carlo_ml"
        )
        return MonteCarloML
    except ImportError as e:
    ...
    try:
        ...
        for i in range(5):  # Check up to 5 levels up
            ...
            monte_carlo_ml_path = (
                parent / "src" / "pricing_models" / "monte_carlo_ml.py"
            )
    ...
    try:
        ...
        if cloud_root.exists():
            monte_carlo_ml_path = (
                cloud_root / "src" / "pricing_models" / "monte_carlo_ml.py"
            )
            if monte_carlo_ml_path.exists():
                logger.info(f"Found monte_carlo_ml.py at: {monte_carlo_ml_path}")
    ...
    logger.error(
        "All import strategies failed. Using comprehensive fallback implementation."
    )
    ...
    class MonteCarloML:
        ...
        def __init__(self, num_simulations=50000, num_steps=100, seed=42):
            ...
            self.y_train = None
            logger.info(
                f"Initialized fallback MonteCarloML with {num_simulations} simulations, {num_steps} steps"
            )
            ...
            for _, row in X.iterrows():
                try:
                    ...
                    if S <= 0 or K <= 0 or T <= 0.001 or sigma <= 0.001:
                        logger.warning(
                            f"Invalid parameters in row: S={S}, K={K}, T={T}, sigma={sigma}"
                        )
                        prices.append(0.0)
                        deltas.append(0.5)
                        gammas.append(0.01)
                    ...
                    Z = np.random.standard_normal(
                        (self.num_simulations, self.num_steps)
                    )
                    ...
                    for t in range(1, self.num_steps):
                        drift = (r - q - 0.5 * sigma**2) * dt
                        diffusion = sigma * np.sqrt(dt) * Z[:, t]
                        S_paths_down[:, t] = S_paths_down[:, t - 1] * np.exp(
                            drift + diffusion
                        )
                    ...
                    for t in range(1, self.num_steps):
                        drift = (r - q - 0.5 * sigma**2) * dt
                        diffusion = sigma * np.sqrt(dt) * Z[:, t]
                        S_paths_up[:, t] = S_paths_up[:, t - 1] * np.exp(
                            drift + diffusion
                        )
        ...
        def predict(self, X):
            ...
            for _, row in X.iterrows():
                try:
                    ...
                    if S <= 0 or K <= 0 or T <= 0.001 or sigma <= 0.001:
                        logger.warning(
                            f"Invalid prediction parameters: S={S}, K={K}, T={T}, sigma={sigma}"
                        )
                        prices.append(0.0)
                        deltas.append(0.5)
                        gammas.append(0.01)
            ...
            return pd.DataFrame({"price": prices, "delta": deltas, "gamma": gammas})
...
def greeks_mc_delta_gamma(
    ...
    try:
        p_down = price_monte_carlo(
            S - h, K, T, r, sigma, option_type, q, num_sim, num_steps, seed, use_numba
        )
...
# PAGE CONFIGURATION
# ======================
st.set_page_config(page_title="Monte Carlo ML Surrogate", layout="wide", page_icon="🤖")
    ...
    try:
        ...
        fig = go.Figure(
            data=go.Heatmap(
                ...
                colorscale="RdBu",
                zmid=0,
                colorbar=dict(title=dict(text="Error", side="right"), thickness=30),
            )
        )
...
def create_error_distribution(err_price):
    ...
    try:
        ...
        # Add zero error line
        fig.add_vline(
            x=0, line_dash="dash", line_color="#F87171", annotation_text="Zero Error"
        )
...
def create_sensitivity_chart(
    x_values, y_values, x_label, y_label, current_x=None, title=None
):
...
# ======================
st.markdown(
    '<h1 class="main-header">Monte Carlo ML Surrogate</h1>', unsafe_allow_html=True
)
st.markdown(
    '<p class="sub-header">Machine learning accelerated option pricing with gradient boosting</p>',
    unsafe_allow_html=True,
)
...
with col1:
    ...
    num_sim = st.slider(
        "Simulations (MC target gen)", 10000, 100000, 30000, step=5000, key="sim_ml"
    )
...
with col3:
    st.markdown(
        '<h4 class="metric-label">Fixed Parameters</h4>', unsafe_allow_html=True
    )
    ...
    sigma_fixed = st.slider(
        "Volatility (σ)", 0.05, 0.8, 0.20, step=0.01, key="sigma_ml"
    )
...
st.markdown(
    '<h3 class="subsection-header">Prediction Inputs</h3>', unsafe_allow_html=True
)
col1, col2 = st.columns([1, 1])
with col1:
    st.markdown(
        '<h4 class="metric-label">Price Parameters</h4>', unsafe_allow_html=True
    )
    S = st.number_input(
        "Spot Price (S)", min_value=1.0, value=100.0, step=1.0, key="spot_ml"
    )
    K = st.number_input(
        "Strike Price (K)", min_value=1.0, value=100.0, step=1.0, key="strike_ml"
    )
    T = st.number_input(
        "Time to Maturity (T)", min_value=0.01, value=1.0, step=0.01, key="time_ml"
    )
with col2:
    st.markdown(
...
train = st.button(
    "Fit Surrogate & Compare", type="primary", use_container_width=True, key="train_ml"
)
st.markdown("</div>", unsafe_allow_html=True)
...
if train:
    try:
        ...
        if ml is None:
            st.warning(
                "ML surrogate model is not available. Using fallback implementation."
            )
        ...
        try:
            ...
            price_ml = (
                pred_df["price"].iloc[0]
                if "price" in pred_df and not pd.isna(pred_df["price"].iloc[0])
                else 0.0
            )
            delta_ml = (
                pred_df["delta"].iloc[0]
                if "delta" in pred_df and not pd.isna(pred_df["delta"].iloc[0])
                else 0.5
            )
            gamma_ml = (
                pred_df["gamma"].iloc[0]
                if "gamma" in pred_df and not pd.isna(pred_df["gamma"].iloc[0])
                else 0.01
            )
        except Exception as e:
        ...
        col1.markdown(
            '<div class="metric-label">MC Price</div>', unsafe_allow_html=True
        )
        col1.markdown(
            f'<div class="metric-value">${price_mc:.6f}</div>', unsafe_allow_html=True
        )
        col1.markdown(
            f'<div class="metric-delta">{t_mc_ms:.1f} ms | {num_sim:,} paths</div>',
            unsafe_allow_html=True,
        )

        col2.markdown(
            '<div class="metric-label">ML Price</div>', unsafe_allow_html=True
        )
        col2.markdown(
            f'<div class="metric-value">${price_ml:.6f}</div>', unsafe_allow_html=True
        )
        col2.markdown(
            f'<div class="metric-delta">{t_ml_ms:.3f} ms | Error: {price_error:.6f}</div>',
            unsafe_allow_html=True,
        )

        col3.markdown(
            '<div class="metric-label">ML Delta</div>', unsafe_allow_html=True
        )
        col3.markdown(
            f'<div class="metric-value">{delta_ml:.4f}</div>', unsafe_allow_html=True
        )
        col3.markdown(
            f'<div class="metric-delta">Error: {delta_error:.4f}</div>',
            unsafe_allow_html=True,
        )

        col4.markdown(
            '<div class="metric-label">ML Gamma</div>', unsafe_allow_html=True
        )
        col4.markdown(
            f'<div class="metric-value">{gamma_ml:.6f}</div>', unsafe_allow_html=True
        )
        col4.markdown(
            f'<div class="metric-delta">Error: {gamma_error:.6f}</div>',
            unsafe_allow_html=True,
        )
        ...
        if (
            prices_ml is None
            or prices_mc is None
            or len(prices_ml) == 0
            or len(prices_mc) == 0
        ):
            st.error(
                "Critical error: price calculations returned invalid results. Using zero values instead."
            )
            prices_ml = np.zeros(len(df))
        ...
        with tab1:
            ...
            st.markdown(
                '<p class="chart-description">Comparison of Monte Carlo and ML surrogate pricing for the selected input parameters</p>',
                unsafe_allow_html=True,
            )

            # Create comparison chart with error handling
            fig_comparison = create_comparison_chart(
                price_mc, price_ml, t_mc_ms, t_ml_ms
            )
            st.plotly_chart(fig_comparison, use_container_width=True)

            # Add metrics table
            st.markdown(
                '<h3 class="subsection-header">Prediction Metrics</h3>',
                unsafe_allow_html=True,
            )
            ...
            if option_type == "call":
                ...
                metrics_data["ML Surrogate"].extend(
                    [f"{delta_ml:.4f}", f"{gamma_ml:.6f}"]
                )
                metrics_data["Absolute Error"].extend(
                    [f"{delta_error:.4f}", f"{gamma_error:.6f}"]
                )
            ...
            # Model information
            st.markdown(
                '<h3 class="subsection-header">Model Information</h3>',
                unsafe_allow_html=True,
            )
        ...
        with tab2:
            st.markdown(
                '<h2 class="chart-title">Error Heatmap</h2>', unsafe_allow_html=True
            )
            st.markdown(
                '<p class="chart-description">Visualization of the price error (ML - MC) across the training grid for call options</p>',
                unsafe_allow_html=True,
            )
            ...
            # Error metrics
            st.markdown(
                '<h3 class="subsection-header">Error Statistics</h3>',
                unsafe_allow_html=True,
            )
            ...
            col1.markdown(
                '<div style="background-color: #1E293B; border-radius: 12px; padding: 1.5rem;">',
                unsafe_allow_html=True,
            )
            col1.markdown(
                '<div style="color: #94A3B8; font-size: 0.9rem; font-weight: 500;">Mean Absolute Error</div>',
                unsafe_allow_html=True,
            )
            col1.markdown(
                f'<div style="color: white; font-size: 2rem; font-weight: 700; margin-top: 0.3rem;">{mean_abs_error:.6f}</div>',
                unsafe_allow_html=True,
            )
            ...
            col2.markdown(
                '<div style="color: #94A3B8; font-size: 0.9rem; font-weight: 500;">Max Absolute Error</div>',
                unsafe_allow_html=True,
            )
            col2.markdown(
                f'<div style="color: white; font-size: 2rem; font-weight: 700; margin-top: 0.3rem;">{max_abs_error:.6f}</div>',
                unsafe_allow_html=True,
            )
            ...
            col3.markdown(
                '<div style="color: #94A3B8; font-size: 0.9rem; font-weight: 500;">RMSE</div>',
                unsafe_allow_html=True,
            )
            col3.markdown(
                f'<div style="color: white; font-size: 2rem; font-weight: 700; margin-top: 0.3rem;">{rmse:.6f}</div>',
                unsafe_allow_html=True,
            )
            ...
            col4.markdown(
                '<div style="color: #94A3B8; font-size: 0.9rem; font-weight: 500;">Std Dev of Error</div>',
                unsafe_allow_html=True,
            )
            col4.markdown(
                f'<div style="color: white; font-size: 2rem; font-weight: 700; margin-top: 0.3rem;">{np.std(err_price):.6f}</div>',
                unsafe_allow_html=True,
            )
        ...
        with tab3:
            ...
            st.markdown(
                '<p class="chart-description">How model accuracy varies with different input parameters</p>',
                unsafe_allow_html=True,
            )

            # Analyze error sensitivity to S
            st.markdown(
                '<h3 class="subsection-header">Error vs Spot Price (S)</h3>',
                unsafe_allow_html=True,
            )
            ...
            st.markdown(
                '<h3 class="subsection-header">Error vs Strike Price (K)</h3>',
                unsafe_allow_html=True,
            )
            ...
            # Moneyness analysis
            st.markdown(
                '<h3 class="subsection-header">Error vs Moneyness (S/K)</h3>',
                unsafe_allow_html=True,
            )
        ...
        with tab4:
            ...
            st.markdown(
                '<p class="chart-description">Speed and accuracy comparison between Monte Carlo and ML surrogate methods</p>',
                unsafe_allow_html=True,
            )

            # Speed comparison
            st.markdown(
                '<h3 class="subsection-header">Speed Comparison</h3>',
                unsafe_allow_html=True,
            )
            ...
            **Note**: The speed advantage of the ML surrogate becomes more pronounced with:
            ...
            st.markdown(
                '<h3 class="subsection-header">Accuracy-Speed Tradeoff</h3>',
                unsafe_allow_html=True,
            )
    ...
    st.markdown(
        "Configure your parameters above and click **Fit Surrogate & Compare** to see results"
    )
    st.image(
        "https://images.unsplash.com/photo-1551288049-bebda4e38f71?ixlib=rb-4.0.3&auto=format&fit=crop&w=1000&q=80",
        use_column_width=True,
        caption="Machine learning surrogates accelerate Monte Carlo pricing while maintaining accuracy",
    )
    st.markdown("</div>", unsafe_allow_html=True)
