...
def get_mc_ml_surrogate(
    ...
    try:
        return MonteCarloML(
            num_simulations=int(num_sim), num_steps=int(num_steps), seed=seed
        )
    except Exception as e:
...
def load_readme(max_lines: int = 80) -> str:
    try:
        ...
        for path in possible_paths:
            if path.exists():
                return "".join(
                    path.read_text(encoding="utf-8").splitlines(True)[:max_lines]
                )
...
def price_monte_carlo(
    ...
    try:
        discounted = _simulate_payoffs_fallback(
            S,
            K,
            T,
            r,
...
def greeks_mc_delta_gamma(
    ...
    try:
        disc_down = _simulate_payoffs_fallback(
            S - h,
            K,
            T,
            r,
        ...
        disc_mid = _simulate_payoffs_fallback(
            S,
            K,
            T,
            r,
        ...
        disc_up = _simulate_payoffs_fallback(
            S + h,
            K,
            T,
            r,
