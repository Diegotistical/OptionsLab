...
def price_monte_carlo(S: float, K: float, T: float, r: float, sigma: float, option_type: Literal["call","put"], q: float = 0.0, num_sim: int = 50_000, num_steps: int = 100, seed: Optional[int] = 42, use_numba: bool = False) -> Optional[float]:
...
def greeks_mc_delta_gamma(S: float, K: float, T: float, r: float, sigma: float, option_type: Literal["call","put"], q: float = 0.0, num_sim: int = 50_000, num_steps: int = 100, seed: Optional[int] = 42, h: float = 1e-3, use_numba: bool = False) -> Tuple[Optional[float],Optional[float]]:
    mc = get_mc_pricer(num_sim=num_sim, num_steps=num_steps, seed=seed, use_numba=use_numba)
    if mc is None:
        return None, None
