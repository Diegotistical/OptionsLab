...
# =============================================================================
def monte_carlo_price_single(
    S, K, T, r, sigma, option_type="call", q=0.0, n_paths=10000, seed=42
):
    """
...
class TestAccuracyComparison:
    ...
    def trained_surrogate(self):
        ...
        surrogate.fit(
            n_samples=20000, option_type="call", verbose=False
    ...
    def test_accuracy_across_spots(self, trained_surrogate):
        ...
        for S in spots:
            ...
            error_pct = (
                abs(ml_price - bs_price) / bs_price * 100 if bs_price > 0.01 else 0
            )
            errors.append(error_pct)

            print(
                f"{S:<10.1f} ${bs_price:<11.4f} ${ml_price:<11.4f} {error_pct:<10.2f}%"
            )
    ...
    def test_monte_carlo_vs_black_scholes(self):
        ...
        assert (
            abs(mc_100k - bs_price) / bs_price < 0.02
        ), "MC 100K should be within 2% of BS"
