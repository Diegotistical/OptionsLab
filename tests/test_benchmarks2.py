...
mc_pricer_cpu = MonteCarloPricer(num_simulations=num_simulations, use_numba=True, use_gpu=False)
mc_pricer_gpu = MonteCarloPricer(num_simulations=num_simulations, use_numba=True, use_gpu=True)
...
print(f"Monte Carlo CPU | Price: {mc_price_cpu:.4f} | Delta: {delta_cpu:.4f} | Gamma: {gamma_cpu:.6f} | Time: {mc_cpu_time:.2f} ms")
...
if mc_pricer_gpu.use_gpu:
    ...
    print(f"Monte Carlo GPU | Price: {mc_price_gpu:.4f} | Delta: {delta_gpu:.4f} | Gamma: {gamma_gpu:.6f} | Time: {mc_gpu_time:.2f} ms")
else:
    mc_gpu_time = None
...
X_train = np.column_stack([S_grid, np.full(10,K), np.full(10,T), np.full(10,r), np.full(10,sigma), np.full(10,q)])
y_train = np.array([ [mc_pricer_cpu.price(s,K,T,r,sigma,option_type,q),
                      mc_pricer_cpu.delta_gamma(s,K,T,r,sigma,option_type,q)[0],
                      mc_pricer_cpu.delta_gamma(s,K,T,r,sigma,option_type,q)[1] ] for s in S_grid])

ml_model = MLSurrogate(Pipeline([
    ("scaler", StandardScaler()),
    ("regressor", MultiOutputRegressor(GradientBoostingRegressor(n_estimators=200, max_depth=5, random_state=42)))
]))
start = time.perf_counter()
...
print(f"ML Surrogate    | Price: {ml_pred[0,0]:.4f} | Delta: {ml_pred[0,1]:.4f} | Gamma: {ml_pred[0,2]:.6f} | Fit: {ml_fit_time:.2f} ms | Predict: {ml_predict_time:.2f} ms")
