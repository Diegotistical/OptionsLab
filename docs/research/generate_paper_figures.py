...
def generate_epp_vs_dropout():
    ...
    ax.semilogy(dropout_pct, epp_svi + 0.01, 'r-o', label='SVI', linewidth=2, markersize=6)
    ax.semilogy(dropout_pct, epp_ssvi + 0.01, '-s', color='orange', label='SSVI', linewidth=2, markersize=6)
    ax.semilogy(dropout_pct, epp_mlp, 'b-^', label='MLP (unconstrained)', linewidth=2, markersize=6)
    ax.semilogy(dropout_pct, epp_soft + 0.01, 'g-d', label='MLP-Soft (λ=0.1)', linewidth=2, markersize=6)
    ax.semilogy(dropout_pct, epp_cinn + 0.01, 'k-*', label='CINN (Ours)', linewidth=2.5, markersize=10)
    
    # Add horizontal line at 0.1 bps (negligible arbitrage threshold)
    ax.axhline(y=0.1, color='gray', linestyle='--', alpha=0.7, label='Negligible threshold (0.1 bps)')
    ...
    # Add annotation
    ax.annotate('CINN maintains\nnear-zero EPP', xy=(55, 0.05), fontsize=9,
                ha='center', style='italic')
    ax.annotate('SVI breaks under\nsparsity', xy=(60, 4), fontsize=9,
...
def generate_surface_comparison():
    ...
    IV_true = np.sqrt(W_true / TT) * 100  # Convert to IV %
    ...
    W_svi = svi_smile(K, a=0.038, b=0.14, rho=-0.25, m=0.02, sigma=0.22) * np.sqrt(TT) + noise_svi
    ...
    for i, (title, IV, cmap) in enumerate(zip(titles, surfaces, colors)):
        ...
        surf = ax.plot_surface(K, TT, IV, cmap=cmap, alpha=0.85, 
                               linewidth=0.2, antialiased=True, edgecolor='gray')
        ...
        for ti in [0.1, 0.5, 1.0]:
            ...
            ax.scatter(k[obs_idx], [ti]*len(obs_idx), IV[t_idx, obs_idx], 
                      c='black', s=20, marker='o', depthshade=False)
...
def generate_density_heatmaps():
    ...
    im1 = axes[0].pcolormesh(K, TT, g_mlp, cmap='RdYlBu', norm=norm, shading='auto')
    ...
    axes[0].text(0.02, 0.98, f'Violations: {violation_pct:.1f}%', 
    ...
    axes[1].contour(K, TT, g_cinn, levels=[0], colors='black', linewidths=2, linestyles='--')
    ...
    axes[1].text(0.02, 0.98, f'Violations: {violation_pct_cinn:.1f}%', 
